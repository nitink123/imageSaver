...
class MainActivity : AppCompatActivity() {
    ...
    private fun saveImage(responseList: List<ApiResponse>, albumId: Int, numPhotos: Int) {
        ...
        for (albumIdToSave in 1..albumId) {
            val photosForAlbum = photosByAlbum[albumIdToSave]?.distinctBy { it.url }
                ?.take(numPhotos) // Get the distinct photos for the album and limit to numPhotos
            if (photosForAlbum != null) {
                for (photoResponse in photosForAlbum) {
                    ...
                    lifecycleScope.launch {
                        ...
                        val fileName =
                            "image_${photoResponse.albumId}_${photoResponse.id}.jpg" // Generate a unique file name for each image
